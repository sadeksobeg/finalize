...
import { TimelineService } from 'src/app/studio/services/timeline.service';
...
export class ListMediasComponent implements OnInit {
  ...
  addToTimeline(media: MediaDto): void {
    // Find the first video track or create one
    const timeline = this.timelineService['timelineSubject'].value;
    let track = timeline.videoTracks[0];
    if (!track) {
      this.timelineService.addVideoTrack();
      track = this.timelineService['timelineSubject'].value.videoTracks[0];
    }

    // Create a VideoClip from MediaDto
    const newClip = {
      id: 'clip_' + Date.now(),
      name: media.title || 'Untitled',
      resource: media.video || '',
      in: 0,
      out: 10, // or set based on media duration if available
      start: 0,
      duration: 10, // or set based on media duration if available
      trackIndex: 0,
      selected: false,
      filters: [],
      properties: {
    ...
    this.timelineService.addClipToTrack(newClip, track.id);
  }
